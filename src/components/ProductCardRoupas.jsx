...
const ProductCardRoupas = ({ limit = 4 }) => {
  ...
  const handleAddToCart = (product) => {
    ...
    console.log("Produto de roupa adicionado ao carrinho:", product);
  };
  
  
  const cardsDataRoupa = [
        {
      id: '17',
    ...
    {
      id: '28',
      imageUrl: 'https://static.nike.com/a/images/t_PDP_1728_v1/f_auto,q_auto:eco,u_126ab356-44d8-4a06-89b4-fcdcc8df0245,c_scale,fl_relative,w_1.0,h_1.0,fl_layer_apply/e82c2bc1-5a6f-4cbc-905f-f99a81f23ef0/jordan-heritage-womens-t-shirt-WvMzWd.png',
      productName : 'Jordan Heritage T-shirt',
      price: '899'
    },
  ...
  const limitedCardsDataRoupa = cardsDataRoupa.slice(0, limit);

  return (
    <div>
      <div className="grid sm:grid-cols-4 grid-cols-2 gap-x-5 gap-y-12 md:gap-y-14 ">
        {limitedCardsDataRoupa.map((card) => (
          <div key={card.id} id={card.id} className="cursor-pointer border z-50 border-gray-300 relative">
            <img
              src={card.imageUrl}
              alt=""
            ...
            <div className="absolute inset-0 flex items-center  justify-between flex-col">
              <p className="py-1 border border-gray-400 lg:min-h-0 min-h-12 bg-zinc-700 w-full justify-center flex items-center text-center text-sm text-white font-semibold sm:text-md lg:text-lg">{card.productName}</p>
              <div className='text-white pl-2  border border-gray-400 bg-zinc-700 flex w-full items-center justify-between sm:text-md font-normal lg:text-xl'>
