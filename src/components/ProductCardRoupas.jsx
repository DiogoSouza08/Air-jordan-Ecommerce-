...
const ProductCardRoupas = ({ limit = 4 }) => {
  ...
  const filteredCardsDataRoupa = filter === 'all' ? cardsDataRoupa : cardsDataRoupa.filter(product => filter === 'feminino' ? !product.gender || product.gender === 'feminino' : product.gender === 'masculino');

  const limitedFilteredCardsDataRoupa = filteredCardsDataRoupa.slice(0, limit);


  return (
    <div>
      <div className=' gap-2 items-center flex  mb-6 justify-end'>
      ...
      <div className="grid sm:grid-cols-4 grid-cols-2 gap-x-5 gap-y-12 md:gap-y-14">
        {limitedFilteredCardsDataRoupa.map((card) => (
